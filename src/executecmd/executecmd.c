...
/*   Updated: 2023/09/13 15:10:05 by minakim          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	executecmd(t_deque *deque, t_elst *lst)
{
	...
	while (0 < deque->size)
	{
		n = deque_pop_back(deque);
		while (exe_f[++i].cmd_func != NULL)
		{
			if (n->input_flag == exe_f[i].flag && exe_f[i].type == INPUT)
				exe_f[i].cmd_func(n, lst);
			else if (n->output_flag == exe_f[i].flag && exe_f[i].type == OUTPUT)
				exe_f[i].cmd_func(n, lst);
		}
	}
	return ;
